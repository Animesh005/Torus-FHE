...
io = open("../../test_results/KMS/logs/log_KMS_errors_wo_FFT.log", "w+")
logger = SimpleLogger(io)

function main()

    parties_set=[2] # different number of parties that are tested

    params_set = [mktfhe_parameters_2party_fast]
    ...
    for k=beginning:length(params_set)
        ...
        function keygen()
            ...
            secret_keys, MKCloudKey_new_wo_FFT(ck_parts, shared_key)
        end
        ...
        for trial = 1:number_trials
            ...
            bootstrap_time[trial*2-1] =  @elapsed enc_mess1=mk_bootstrap_new_wo_FFT(cloud_key.bootstrap_key,cloud_key.keyswitch_key,encode(1,8),enc_mess1,true)
            bootstrap_time[trial*2] = @elapsed enc_mess2=mk_bootstrap_new_wo_FFT(cloud_key.bootstrap_key,cloud_key.keyswitch_key,encode(1,8),enc_mess2,true)
        ...
        writedlm("../../test_results/KMS/mk-noises__scheme-2_parties-"*string(parties)*"_lambda-100_pi-2_qw-2_sf-4.00_wo_FFT.dat", noise_results)

        if beginning==1

            open("../../test_results/KMS/mk_bootstrap_time_min_2_wo_FFT.dat","w+") do file
            ...
            open("../../test_results/KMS/mk_bootstrap_time_median_2_wo_FFT.dat","w+") do file
            ...
            open("../../test_results/KMS/mk-positive_errors_2_wo_FFT.dat","w+") do file
            ...
            open("../../test_results/KMS/mk-negative_errors_2_wo_FFT.dat","w+") do file
            ...
            open("../../test_results/KMS/mk_bootstrap_time_min_2_wo_FFT.dat","a+") do file
                writedlm(file,bootstrap_time_min_list[beginning:k])
            end

            open("../../test_results/KMS/mk_bootstrap_time_median_2_wo_FFT.dat","a+") do file
                writedlm(file,bootstrap_time_median_list[beginning:k])
            end

            open("../../test_results/KMS/mk-positive_errors_2_wo_FFT.dat","a+") do file
                writedlm(file,wrong_phase_after_rounding_positive_results[beginning:k])
            end

            open("../../test_results/KMS/mk-negative_errors_2_wo_FFT.dat","a+") do file
                writedlm(file,wrong_phase_after_rounding_negative_results[beginning:k])
            end
        end
