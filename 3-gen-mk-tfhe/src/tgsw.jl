...
struct TGswSample
    ...
    samples :: Array{RLweSample, 2} # array of size (decomp_length, mask_size+1)

    function TGswSample(tgsw_params::TGswParams, samples::Array{RLweSample, 2})
...
struct TransformedTGswSample
    tgsw_params :: TGswParams
    rlwe_params :: RLweParams
    samples :: Array{TransformedRLweSample, 2} # array of size (decomp_length, mask_size+1)

    TransformedTGswSample(tgsw_params::TGswParams, samples::Array{TransformedRLweSample, 2}) =
        new(tgsw_params, samples[1].params, samples)
end
...
The dimensions of the RLWE sample array in the TGSW sample correspond to the first two indices
(`i` and `j`), and the index `k` corresponds to the vector of polynomials in a single RLWE sample.
"""
function tgsw_add_gadget_times_message(sample::TGswSample, message::Union{Int32, Int64})
    ...
    # types for zero RLWE samples and zero polynomials, but that's just too much infrastructure
...
function tgsw_encrypt_zero(rng::AbstractRNG, alpha::Float64, key::RLweKey, params::TGswParams,wo_FFT::Int64 = 0)
    mask_size = key.params.mask_size
    decomp_length = params.decomp_length
    TGswSample(
        params,
        [rlwe_encrypt_zero(rng, alpha, key, wo_FFT) for i in 1:decomp_length, j in 1:mask_size+1])
end


function tgsw_encrypt(
        rng::AbstractRNG, message::Int32, alpha::Float64, key::RLweKey, params::TGswParams,wo_FFT::Int64=0)
    tgsw_zero = tgsw_encrypt_zero(rng, alpha, key, params,wo_FFT)
...
function tgsw_extern_mul(accum::RLweSample, gsw::TransformedTGswSample)
...
function tgsw_extern_mul_wo_FFT(accum::RLweSample, gsw::TGswSample)
    dec_accum = hcat(decompose.(accum.a, Ref(gsw.tgsw_params))...)
    #tr_dec_accum = forward_transform.(dec_accum, accum.params.is32)
    sum(gsw.samples .* dec_accum)
end
