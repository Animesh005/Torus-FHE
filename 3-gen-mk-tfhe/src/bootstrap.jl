struct BootstrapKey
    ...
    function BootstrapKey(
            ...
            rlwe_key::RLweKey, tgsw_params::TGswParams)

        rlwe_params = rlwe_key.params
        bk = [tgsw_encrypt(rng, key_elem, alpha, rlwe_key, tgsw_params) for key_elem in lwe_key.key]
        transformed_bk = forward_transform.(bk)

        new(tgsw_params, rlwe_params, transformed_bk)
    end
end


function mux_rotate(accum::RLweSample, bki::TransformedTGswSample, barai::Int32)
    # accum += BK_i * [(X^bar{a}_i-1) * accum]
    temp = mul_by_monomial(accum, barai) - accum
    accum + tgsw_extern_mul(temp, bki)
end

function mux_rotate_wo_FFT(accum::RLweSample, bki::TGswSample, barai::Int32)
    # accum += BK_i * [(X^bar{a}_i-1) * accum]
    temp = mul_by_monomial(accum, barai) - accum
    accum + tgsw_extern_mul_wo_FFT(temp, bki)
end


#=
 * multiply the accumulator by X^sum(bara_i.s_i)
 * @param accum the RLWE sample to multiply
 * @param bk An array of n TGSW FFT samples where bk_i encodes s_i
 * @param bara An array of n coefficients between 0 and 2N-1
=#
function blind_rotate(accum::RLweSample, bk::BootstrapKey, bara::Array{Int32, 1})
...
function blind_rotate_and_extract(
    ...
    accum = rlwe_noiseless_trivial(testvectbis, bk.rlwe_params)
    accum = blind_rotate(accum, bk, bara)
    rlwe_extract_sample(accum)
end
